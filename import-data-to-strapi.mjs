...
// 创建或获取目的地
async function createOrGetDestination(destinationName, locale) {
  try {
    // 先检查目的地是否已存在
    const response = await axios.get(
      `${STRAPI_URL}/api/destinations?filters[destinationname][$eq]=${encodeURIComponent(destinationName)}&filters[locale][$eq]=${locale}`,
      { headers: getHeaders() }
    );

    if (response.data.data && response.data.data.length > 0) {
      console.log(`Destination ${destinationName} already exists with ID: ${response.data.data[0].id}`);
      return response.data.data[0];
    }
    ...
    console.log(`Created destination ${destinationName} with ID: ${createResponse.data.data.id}`);
    return createResponse.data.data;
  } catch (error) {
    console.error(`Error creating/getting destination ${destinationName}:`, error.message);
    if (error.response) {
      console.error('Response data:', error.response.data);
    }
    return null;
...
async function createChineseProgram(programId) {
  try {
    ...
    // 创建或获取国家
    const country = await createOrGetCountry(programData.country_zh, 'zh');
    
    // 创建或获取目的地
    const destination = await createOrGetDestination(programData.destination_zh, 'zh');
    ...
    // 添加关联
    if (country) {
      createData.data.countries = [country.id];
    }
    
    if (destination) {
      createData.data.destinations = [destination.id];
    }
...
async function createEnglishProgram(programId, documentId) {
  try {
    ...
    // 创建或获取国家
    const country = await createOrGetCountry(programData.country_en, 'en');
    
    // 创建或获取目的地
    const destination = await createOrGetDestination(programData.destination_en, 'en');
    
    // 获取年级ID（注意：年级可能需要先创建英文版本）
