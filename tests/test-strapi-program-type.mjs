// 测试 Strapi API 获取 program_type 字段的脚本
...
async function fetchBlogData() {
  try {
    ...
    // 获取博客列表，明确指定要填充的字段
    const populateFields = ['image', 'grade', 'program_type', 'Slideshow', 'localizations'];
    const populateQuery = populateFields.map(field => `populate=${field}`).join('&');
    
    const listUrl = `${strapiConfig.blogPostsEndpoint}?${populateQuery}`;
    ...
    // 保存博客列表数据
    fs.writeFileSync(
      path.join(outputDir, 'blog-list-with-fields.json'), 
      JSON.stringify(listResponse.data, null, 2)
    );
    console.log(`博客列表数据已保存到 ${path.join(outputDir, 'blog-list-with-fields.json')}`);
    ...
    if (blogs.length > 0) {
      ...
      // 保存第一篇博客的详细信息
      fs.writeFileSync(
        path.join(outputDir, 'first-blog-with-fields.json'), 
        JSON.stringify(firstBlog, null, 2)
      );
      console.log(`第一篇博客详细信息已保存到 ${path.join(outputDir, 'first-blog-with-fields.json')}`);
...
function analyzeAllFields(blog) {
  ...
  // 检查 grade 字段
  if (blog.attributes && blog.attributes.grade) {
    console.log('\n检查 grade 字段:');
    const grade = blog.attributes.grade;
    console.log(`- 类型: ${Array.isArray(grade) ? 'array' : typeof grade}`);
    
    if (typeof grade === 'object' && grade !== null) {
      if (grade.data) {
        console.log(`- data 类型: ${Array.isArray(grade.data) ? 'array' : typeof grade.data}`);
        
        if (grade.data && grade.data.attributes) {
          console.log('- 属性:');
          Object.keys(grade.data.attributes).forEach(key => {
            const value = grade.data.attributes[key];
            const type = Array.isArray(value) ? 'array' : typeof value;
            console.log(`  - ${key}: ${type} ${value === null ? '(null)' : (typeof value === 'string' ? `= "${value}"` : '')}`);
          });
        }
