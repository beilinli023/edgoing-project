  ...
  try {
    ...
    if (response.data.data && response.data.data.length > 0) {
      ...
      for (const chineseProgram of response.data.data) {
        ...
        const title = chineseProgram.attributes.title;
        const programId = chineseProgram.attributes.program_id;
        ...
        try {
          ...
          console.error(`Error checking existing English program ${programId}:`, error.message);
        }
